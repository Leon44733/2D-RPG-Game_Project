 ...
 *  Author: lbarwe
 *  -----
 *  Last Modified: Sa 08.March 2025, 8:00:51 pm
 *  Modified By: lbarwe
 *  -----
...
namespace kernel
{
    GameLoop::GameLoop(std::shared_ptr<ui::Window> aSDLWindow, std::unique_ptr<gfx::render::RendererManager> aRManager, std::unique_ptr<gfx::texture::TextureManager> aTManager)  :
        ...
        {
            // add player camera
            mCamera = std::make_shared<Camera>(aSDLWindow.get()->getWidth(), aSDLWindow.get()->getHeight(), mRManager->getBgElement("Background_1").getElementWidth(), mRManager->getBgElement("Background_1").getElementHeigth());
        }
    ...
    {              
        ...
        // mCamera->update(mPlayer.getElementPos().x, mPlayer.getElementPos().y);

        mRManager->renderBgElement("Background_1", mCamera);
        mRManager->renderAllGuiElem();
        mRManager->renderCharElement("Player", mCamera);
